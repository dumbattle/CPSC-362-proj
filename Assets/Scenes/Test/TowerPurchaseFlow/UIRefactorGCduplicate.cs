...
using UnityEngine.UI;
    ...
    public TowerUIManagerRefactor tui;
    ...
    GameState SceneStartState() {
        ...
        PlayPauseUIManager.SetWinLosePanel(false);
    ...
    GameState WaveSpawnState() {
        TowerUIManagerRefactor.SetPathIndicator(false);
        ws.SpawnUpdate();
        GameplayUpdate();

        if (em.health <= 0) {
            PlayPauseUIManager.SetWinLosePanel(true);
    ...
    GameState WaveEndState() {
        currentWave++;
        PlayPauseUIManager.SetStartWave(true);
